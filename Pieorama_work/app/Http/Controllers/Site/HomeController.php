...
class HomeController extends Controller
{
    ...
    $user_ip=$request->ip();
      ...
      try {
		...
		$channels = PieChannel::where('is_deleted',0)->where('status',1)->get();	

        return view('Site.index.index',compact('MainPagevd','MainPageThumb','MainPagebackgroundColor','welcome_message','background_color','foreground_color','cookie_value','Allgif','moregif', 'channels'));
     ...
     //  {
      // dd($request->all());
       if($request->channel != 0){
       	 //dd($request->all());
	   		$channels = PieChannel::where('id',$request->channel)->where('is_deleted',0)->where('status',1)->first();
	   		$channel_data = $channels->tags_id;
	   		$channeldata = json_decode($channel_data);
	   		

	   		$AllVideos = PieVideo::leftjoin('sc_pie_tags', 'sc_pie_tags.video_id', '=', 'sc_pie_video.id')
							->where('sc_pie_video.status',1)
							->whereIn('sc_pie_tags.id',$channeldata)
							->whereNotNull('sc_pie_video.video_thumbnail_file_path')
							->where('sc_pie_video.video_thumbnail_file_path','!=','')
							->where('sc_pie_video.is_deleted',0)
							
							->orderBy('sc_pie_video.profiled_pieogram','DESC')
			...
			//dd($AllVideos);
       		...
       		$channels = PieChannel::where('is_deleted',0)->where('status',1)->get();
	   		 $AllVideos = PieVideo::where('status',1)
	   		 				->whereNotNull('video_thumbnail_file_path')
							->where('video_thumbnail_file_path','!=','')
			    			->where('is_deleted',0)
			    			->orderBy('profiled_pieogram','DESC')
			    			->orderBy('id','DESC')
			    			->skip($request->start)
      ...
      // }
		...
		foreach($AllVideos as $key=>$row)
		{
		  ...
		  //  $dd = strlen( $title);
			...
			$userprofile_id=base64_encode($user->id??0);
			$row['userprofile_id']=urlencode(base64_encode($userprofile_id));
			$row['userprofile_id'] = str_replace('%3D%3D', '', $row['userprofile_id']);
...
function videoplayontimer(Request $request){
	...
	function gifdata(Request $request){
		...
		if(!empty(auth()->user()->id)){
		 ...
		 $Allgif = PieVideo::where('status',1)
    			->where('is_deleted',0)
    			->whereNotNull('video_animated_file_path')
    			->where('video_animated_file_path','!=','')
    			->orderBy('profiled_pieogram','DESC')
    			->orderBy('id','DESC')
    			->skip($request->start)
    			->take($request->end - $request->start)
	 ...
	 public function getDownload($id){
		...
		$getgif = PieVideo::where('id',$id)
    			 ...
    			 ->where('status',1)
    			 ->first();
    	$document = $getgif->video_animated_file_path;
    	$gif_id = $getgif->id;
		...
		//print_r($browser); die;
    	 ...
    	 header('Content-Type: application/force-download');
			...
			// It will be called pieorama.gif
