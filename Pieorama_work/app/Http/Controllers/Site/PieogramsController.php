...
class PieogramsController extends Controller
{
    ...
    { 
		...
		$user_verify=new UserVerification; 
		if(isset($value)){
			$AllVideos = PieVideo::where('status',1)->where('is_deleted',0)->whereNotIn('id',[$video_id, $value])->orderBy('id','DESC')->take(4)->get();
		}else{
			$AllVideos = PieVideo::where('status',1)->where('is_deleted',0)->whereNotIn('id',[$video_id])->orderBy('id','DESC')->take(4)->get();
		}
