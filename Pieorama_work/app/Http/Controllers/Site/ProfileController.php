...
class ProfileController extends Controller
{
   ...
   // Pieorama user profile
    public function otherUserProfile(Request $request, $user_id){  
       ...
       $user =  User::where('id',$userid)->where('status',1)->where('is_deleted',0)->first();
       $id_exist = Helper::badgedata($userid);
       $userAddress = UserAddress::where('user_id', $userid)->first();
       $videoDetails = PieVideo::where('status',1)->where('is_deleted',0)->where('created_by',$userid)->get();
       
      $user_verify=new UserVerification;
       foreach ($videoDetails as $key => $v) {
       $updated_at=$v['updated_at'];  

         $updated_at=$user_verify->timeago($updated_at);
         $v->updated_time  = $updated_at;
         
         $videoTags = PieTags::where('is_deleted',0)->where('video_id',$v->id)->orderBy('id','ASC')->get(); 
         $tagArray=array();
            if(!empty($videoTags))
            {
      ...
      // dd($videoDetails);
       return view('Site.other-user-profile.other-user-profile',['user'=>$user,'userAddress'=>$userAddress,'videoDetails'=>$videoDetails,'id_exist' => $id_exist]);
   }
