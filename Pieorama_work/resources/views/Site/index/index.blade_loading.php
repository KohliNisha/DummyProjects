			...
			<ul class="christanPieograms-wrapper clearfix" id="results">	
				<div id="all_rows">
					...
					<input type="hidden" id="end" value="8">
					 <div id="loader-icon" style="display:none;"><i class="fa fa-spinner fa-spin" style="font-size:48px;"></i> </div> 
			</ul>
		</div>
</div>
@endsection
 	...
 	$(window).scroll(function() {
		...
		if($(window).scrollTop() >= $(document).height() - 800) {
	   		if(load_more==0){
	    		...
	    		var start = $("#start").val();
	            var end = $("#end").val();
	    	    var start1 = parseInt(start) + 8; 
	    	    var end1 = parseInt(end) + 8; 
	    	    $("#start").val(start1);	 
	    	    $("#end").val(end1);
            } 
		...
		$.ajax({
	        ...
	        $('#loader-icon').hide();
			 ...
			 success: function (response) {
				...
				var duration=response.all_user[i].duration;
				...
				title = title.substring(0, 28);
				 ...
				 str = str + '</div>';
				 str = str + '<div class="clock-count">';
				 str = str + '<i class="fa fa-clock-o" aria-hidden="true"></i>';
				 str = str + '<span class="dislike-count" data-id="1346">'+duration+'s</span>';
				 str = str + '</div>';
				 str = str + '</div>';
				 str = str + '</div>';
				 str = str + '</div>';
