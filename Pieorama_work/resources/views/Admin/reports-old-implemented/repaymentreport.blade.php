      ...
      <nav aria-label="breadcrumb">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="template-demo">
                      ...
                      <a href="/admin/repaymentreports"><button type="button" class="btn btn-primary btn-fw">Repayment</button></a>
       ...
       <div class="card-body" style="padding-top: 0px;">
         <div class="template-demo">
            ...
            <form method="post" style="display: inline;"> @csrf <input type="hidden" value="user" name="alluser"> <button type="submit" style="margin-top: 20px;" class="btn btn-gradient-secondary">All Repayment EMI Report</button></form>
          </div>
        <hr/>
         ...
         <!--  <div class="row" style="margin-left: 10px;">
                <div class="col-md-3 ttnclass" id="custom"> <button class="font-weight-normal mb-3 btn btn-default ">Custom Date </button> </div>
             <div class="col-md-3 ttnclass" id="all"> <form method="post"> @csrf <input type="hidden" value="user" name="alluser"> <button type="submit" class="font-weight-normal mb-3 btn btn-default"> All User</button></form></div> 
                <div class="col-md-3 ttnclass" id="monthly"><button class="font-weight-normal mb-3 btn btn-default">Monthly Report</button></div>
                <div class="col-md-3 ttnclass" id="selectDate"><button class="font-weight-normal mb-3 btn btn-default">Weekly Report</button></div>
                    </div> -->
                    <!-- custom date -->
                  ...
                  <table class="table display table-bordered table-striped" id="example">
                    ...
                    @foreach($data as $key=>$loan) 
                      <tr>
                        ...
                        <td>  {{$loan->emi_no}} </td> 
                   <!--      <td>  {{$loan->emi_no}} </td>  -->  
          <td>  {{$loan->loanapplicationfun->title.' '. $loan->loanapplicationfun->first_name.' '.$loan->loanapplicationfun->last_name}} </td> 
                        <td> GHC {{$loan->paybale_emi}} </td> 
                        <td> {{date('Y/m/d', strtotime($loan->payment_due_date))}}  </td> 

                        <td>  
                         @if($loan->is_paid =='1')                          
                          <label class='badge badge-success cursor' style=''>Paid</label>
                        @endif
                        
                        @if($loan->is_paid =='0')                        
                         <label class='badge badge-danger cursor' style='background-color: #e693a4;'>Pending</label>
                        @endif
                        </td>                        

                        <td> 
                          @if($loan->is_paid =='1') 
                              {{date('Y/m/d', strtotime($loan->paid_at))}} 
                           @endif
                        </td> 
                        ...
                        @if($loan->is_paid =='1')  
                        <td>  
                            @if($loan->updated_by =='0')    
                              
                            @endif

                            @if($loan->updated_by == $loan->user_id) 
                              Online Expresspay
                            @endif


                            @if($loan->updated_by =='1') 
                              Manually by Admin
                            @endif
